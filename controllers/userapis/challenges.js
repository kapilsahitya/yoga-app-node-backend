...
const getTotalDaysCompleted = async (weekId, userId) => {
	const daysCompleted = await yogaworkoutDaysCompleted.find({ week_id: weekId, user_id: userId })
...
const getAllChallenges = async (req, res) => {
	try {
		...
		if (data.user_id && data.user_id != '') {
			...
			if (data.session && data.session != '' && data.device_id && data.device_id != '') {
				const session = data.session;
				const deviceId = data.device_id;
				const checkuserLogin = await checkUserLogin(userId, session, deviceId)
				if (checkuserLogin) {
					let challenges = await yogaworkoutChallenges.find({ isActive: 1 }).sort({ createdAt: -1 });
					if (challenges.length === 0) {
						...
						const challengesWithImages = await Promise.all(
							challenges.map(async (item) => {
								...
								let totalweekanddaycompleted = await getTotalDayCompleted(item._id,userId)
								if (item.image !== '') {
									const imageurl = await getFile(item.image); // Assuming getFile is an async function
									// console.log("imageurl", imageurl);
									return { ...updatedItem, image: imageurl, totalweek: totalweeks.totalWeeks, totaldays: totalweeks.totalday, totaldayscompleted: totalweekanddaycompleted.totalday }; // Update the image URL
								}
								return { ...updatedItem, totalweek: totalweeks.totalWeeks, totaldays: totalweeks.totalday, totaldayscompleted: totalweekanddaycompleted.totalday }; // Return the item unchanged if no image update is needed
							})
						);
						res.status(200).json({
				...
				const challengesWithImages = await Promise.all(
					challenges.map(async (item) => {
						...
						if (item.image !== '') {
							const imageurl = await getFile(item.image); // Assuming getFile is an async function
							// console.log("imageurl", imageurl);
							return { ...updatedItem, image: imageurl, totalweek: totalweeks.totalWeeks, totaldays: totalweeks.totalday, totaldayscompleted: 0 }; // Update the image URL
						}
						return { ...updatedItem, totalweek: totalweeks.totalWeeks, totaldays: totalweeks.totalday, totaldayscompleted: 0 }; // Return the item unchanged if no image update is needed
					})
				);
				res.status(200).json({
					data: { success: 1, challenges: challengesWithImages, error: '' },
